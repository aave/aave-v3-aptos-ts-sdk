...
export type FixedPointNumberValue<
  T extends FixedPointNumber = FixedPointNumber,
> = Numberish | T;
...
export class FixedPointNumber implements IFixedPointNumber {
  ...
  constructor(value: FixedPointNumberValue, scale?: Numberish) {
    if (value instanceof FixedPointNumber) {
      ...
      this.scaleMul = this.scaleMul.bind(this);
      this.scaleDiv = this.scaleDiv.bind(this);
  ...
  add(value: FixedPointNumberValue): FixedPointNumber {
    ...
    if (this.scale > value.scale) {
      return new FixedPointNumber(
        this.value + value.scaleUp(this.scale).value,
        this.scale,
      );
    }
    return new FixedPointNumber(
      this.scaleUp(value.scale).value + value.value,
      value.scale,
    );
  }
  ...
  mul(value: FixedPointNumberValue): FixedPointNumber {
    if (!(value instanceof FixedPointNumber)) {
      const wrappedValue = BigInt(value);
      return new FixedPointNumber(this.value * wrappedValue, this.scale * 2n);
    }
    return new FixedPointNumber(
      this.value * value.value,
      this.scale + value.scale,
  ...
  div(value: FixedPointNumberValue): FixedPointNumber {
    ...
    const dividend =
      this.scale < value.scale ? this.scaleUp(value.scale) : this;
    const dividendWithMinPrecision =
      dividend.scale - value.scale < 3n
        ? this.scaleUp(dividend.scale + 3n)
        : this;
    return new FixedPointNumber(
  ...
  pow(exp: Numberish): FixedPointNumber {
    ...
    return new FixedPointNumber(
      this.value ** BigInt(exp),
      this.scale ** wrappedExp,
    );
  }
  ...
  scaleUp(scale: Numberish): FixedPointNumber {
    ...
    return new FixedPointNumber(
      this.value * 10n ** BigInt(wrappedScale - this.scale),
      scale,
    );
  }
  ...
  scaleDown(scale: Numberish, rounding: RoundMode): FixedPointNumber {
    ...
    const preNewValue =
      this.value / 10n ** BigInt(this.scale - wrappedScale - 1n);
    switch (rounding) {
      case RoundMode.ROUND_DOWN:
        return new FixedPointNumber(preNewValue / 10n, wrappedScale);
      case RoundMode.ROUND_UP:
        return new FixedPointNumber(preNewValue / 10n + 1n, wrappedScale);
      case RoundMode.ROUND_HALF_UP:
        return new FixedPointNumber((preNewValue + 5n) / 10n, wrappedScale);
      case RoundMode.ROUND_HALF_DOWN:
        return new FixedPointNumber((preNewValue + 4n) / 10n, wrappedScale);
    }
  }
  ...
  eq(value: FixedPointNumberValue): boolean {
    if (value instanceof FixedPointNumber) {
      if (this.scale > value.scale) {
        return this.value === value.value * 10n ** (this.scale - value.scale);
      }
      return this.value * 10n ** (value.scale - this.scale) === value.value;
    }
  ...
  percent(percent: number): FixedPointNumber {
    ...
    const scaledPercent = integerPercent * 10n ** this.scale;
    const scaledDivisor = 10n ** (this.scale + BigInt(decimals));
  ...
  format(decimals?: number): string {
    ...
    const positivePart =
      this.value < 0n ? stringValue.substring(1) : stringValue;
    const paddedPositivePart = positivePart.padStart(
      Number(this.scale) + 1,
      "0",
    );
